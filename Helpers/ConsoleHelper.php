...
class ConsoleHelper
{
	...
	/**
	 * Moves the cursor to an absolute position given as column and row by sending ANSI control code CUP or CHA to the
	 * terminal.
	 *
	 * @param int      $column 1-based column number, 1 is the left edge of the screen.
	 * @param int|null $row    1-based row number, 1 is the top edge of the screen. if not set, will move cursor only
	 *                         in current line.
	 */
	...
	/**
	 * Restores the cursor position saved with [[saveCursorPosition()]] by sending ANSI control code RCP to the
	 * terminal.
	 */
	...
	/**
	 * Will show a cursor again when it has been hidden by [[hideCursor()]]  by sending ANSI DECTCEM code ?25h to the
	 * terminal.
	 */
	...
	/**
	 * Clears text from cursor to the beginning of the screen by sending ANSI control code ED with argument 1 to the
	...
	/**
	 * Clears text from cursor position to the beginning of the line by sending ANSI control code EL with argument 1 to
	 * the terminal. Cursor position will not be changed.
	 */
	...
	/**
	 * Clears text from cursor position to the end of the line by sending ANSI control code EL with argument 0 to the
	 * terminal. Cursor position will not be changed.
	 */
			...
			function ( $ansi ) use ( &$tags, $styleMap ) {
				...
				foreach ( explode( ';', $ansi[1] ) as $controlCode ) {
					if ( $controlCode == 0 ) {
						$style = [];
						$reset = true;
					} else if ( $controlCode == self::NEGATIVE ) {
						$negative = true;
					} else if ( isset( $styleMap[ $controlCode ] ) ) {
	 ...
	 *
	 * If screen size could not be detected, or the indentation is greater than the screen size, the text will not be
	 * wrapped.
	 *
	 * The first line will **not** be indented, so `Console::wrapText("Lorem ipsum dolor sit amet.", 4)` will result in
	 * the following output, given the screen width is 16 characters:
	 *
	 * ```
	 * Lorem ipsum
	 *     dolor sit
	...
	public static function select( $prompt, $options = [] ) {
		...
		if ( $input === '?' ) {
			...
			goto top;
		} else if ( ! array_key_exists( $input, $options ) ) {
			goto top;
		}
	 ...
	 *                         the number of characters to display for the progress bar or a float between 0 and 1
	 *                         representing the percentage of screen with the progress bar may take. It can also be set
	 *                         to false to disable the bar and only show progress information like percent, number of
	 *                         items and ETA. If not set, the bar will be as wide as the screen. Screen size will be
	 ...
	 *                                If set to `true`, the line of the progress bar will be cleared. This may also be
	 *                                a string to be displayed instead of the progress bar.
	 * @param bool        $keepPrefix whether to keep the prefix that has been specified for the progressbar when
	 *                                progressbar gets removed. Defaults to true.
	 *
	 * @see startProgress
	 * @see updateProgress
	 */
